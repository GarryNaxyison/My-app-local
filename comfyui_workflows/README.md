...
- `render_poliglot_tech_outro_v15_clean_vector_text.py` - current final compositor that keeps the v14 motion but resolves the URL into one clean deterministic TrueType text layer instead of a per-character final lockup.
...
```powershell
& 'C:\Users\Admin\Documents\ComfyUI\.venv\Scripts\python.exe' .\comfyui_workflows\render_poliglot_tech_outro_v15_clean_vector_text.py
```

Current final output:

```text
C:\Users\Admin\Documents\ComfyUI\output\outro\poliglot_outro_tech_warp_v15_clean_vector_text.mp4
```
