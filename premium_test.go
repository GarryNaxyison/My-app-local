...
func TestPremiumTextUsesInterfaceLanguage(t *testing.T) {
	...
	keyboard := premiumInlineKeyboard(true, localizedPremiumPlans(userState{InterfaceLanguage: "es"}, b.premiumPlanList()), userState{InterfaceLanguage: "es"})
...
func TestPremiumPaymentOptionsKeyboardShowsMethodsForSelectedPlan(t *testing.T) {
	...
	keyboard := premiumPaymentOptionsKeyboard(true, plan, user)
...
func TestReferralInviteTextMentionsFreePremiumWeek(t *testing.T) {
	for _, tc := range []struct {
		code string
		want string
	}{
		{code: "en", want: "free week of Premium"},
		{code: "ru", want: "неделю Premium бесплатно"},
	} {
		copy := premiumUI(userState{InterfaceLanguage: tc.code, InterfaceSelected: true})
		for _, text := range []string{copy.InviteLinkText, copy.ReferralShareText} {
			rendered := fmt.Sprintf(text, "https://example.test/app?ref=CODE")
			if !strings.Contains(rendered, "https://example.test/app?ref=CODE") {
				t.Fatalf("%s invite text does not include link: %q", tc.code, rendered)
			}
			if !strings.Contains(rendered, tc.want) {
				t.Fatalf("%s invite text should mention %q: %q", tc.code, tc.want, rendered)
			}
		}
	}
}
