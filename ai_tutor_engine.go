...
func (e *aiTutorEngine) Submit(ctx context.Context, user userState, sessionID string, input aiTutorSubmitInput) (aiTutorResult, error) {
	...
	if aiTutorStageNeedsChecker(current) {
		if current == aiTutorStageProduction && countAITutorCompleteSentences(answerText) < 2 {
		...
		checkOK := aiTutorCheckerAccepted(checkRaw)
		...
		if !checkOK {
			return aiTutorResult{
				...
				Lesson:   lesson,
				NextStep: aiTutorBuildStep(lesson.Payload, current),
				Feedback: aiTutorFeedback{OK: false, Message: aiTutorCheckerFeedbackMessage(current, checkRaw), JSON: checkRaw},
			}, nil
		}
	...
	if current == aiTutorStageReviewSchedule || next == aiTutorStageComplete {
		...
		if err := e.store.recordAITutorCompletion(user.TelegramID, aiTutorCompletionXP); err != nil {
			return aiTutorResult{}, err
		}
	}
...
func aiTutorCheckerAccepted(raw string) bool {
	var data map[string]any
	if err := json.Unmarshal([]byte(strings.TrimSpace(raw)), &data); err != nil {
		return true
	}
	for _, key := range []string{"ok", "correct"} {
...
func countAITutorCompleteSentences(text string) int {
	...
	for _, char := range text {
		...
		case '.', '!', '?', '。', '！', '？':
			...
			inSentence = false
		default:
			if !inSentence && !strings.ContainsRune(" \t\r\n,;:()[]{}\"'«»", char) {
				inSentence = true
			}
		}
	}
	if count == 0 && len(strings.Fields(text)) > 0 {
		return 1
	}
	return count
}
...
func aiTutorRecallOptions(words []aiTutorWord, correctIndex int) []aiTutorStepOption {
	...
	aiTutorShuffleOptions(options, correct.ID+"|"+itoa(correctIndex))
	return options
}

func aiTutorShuffleOptions(options []aiTutorStepOption, seed string) {
	if len(options) < 2 {
		return
	}
