...
- Original source files are preserved locally in `tools/sources/` (`*.tei` and `kaikki-*.jsonl`) so the dictionaries can be regenerated without hunting for the downloads again.
...
- If a specific word/interface-language prompt is missing, the runtime now uses the nearest local dictionary prompt immediately and warms a precise OpenRouter translation in the background. Successful AI translations are persisted in SQLite in `vocabulary_ai_translations` and mirrored into `vocabulary_translations`, so the next request uses local data instead of another AI call.
