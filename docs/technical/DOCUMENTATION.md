...
Legal pages must keep the same contact/header treatment as Terms, including `@NERIVAapp_bot`, email/contact fields, and dark-theme-safe language selectors. Any new visible public copy must be checked for all 35 supported interface languages and then validated with `node tools/check_encoding_artifacts.mjs`.
...
The web Premium screen opens a Telegram deep link such as `https://t.me/NERIVAapp_bot?start=buy_premium_30d` for Stars payment in a new tab only; the current `/app` page must stay open. When the user opens the bot, the `/start buy_...` payload sends the matching `currency=XTR` invoice. `POST /api/premium/stars` remains available for linked Telegram accounts and also returns the same bot URL after sending an invoice to the bot chat.
...
The referral block is intentionally not duplicated in Settings. Referral sharing uses the selected interface language and addresses the invited person directly: it describes NERIVA, the key bot/web features, and the 7-day Premium bonus received from the link.
